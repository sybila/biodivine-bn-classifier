//! Various functions for loading classification inputs.

use biodivine_lib_param_bn::{BooleanNetwork, ModelAnnotation};

use std::fs::read_to_string;

pub fn extract_properties_from_aeon(
    aeon_string: &str
) -> Result<(Vec<String>, Vec<(String, String)>), String> {
    let annotation = ModelAnnotation::from_model_string(aeon_string);

    let mut assertions: Vec<String> = Vec::new();
    // assertions are expected as:     #!dynamic_assertion: NAME: FORMULA
    ...
    let mut named_properties: Vec<(String, String)> = Vec::new();
    // properties are expected as:     #!dynamic_property: FORMULA
